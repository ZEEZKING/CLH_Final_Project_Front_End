...
let verify = () => {
  ...
  fetch(`https://localhost:7285/api/VerificationCode/VerifyCode/${code}/${id}`)
    .then(res => res.json())
    .then(function (value) {
      if (value.success == false) {
        ...
        location.href = "http://127.0.0.1:5501/AdminFrontEnd/login-form-11/signin.html"
      }
    })
...
let resendCode = () => {
  const options = {
    ...
    fetch(`https://localhost:7285/api/VerificationCode/UpdateCode/${id}`, options)
    .then(res => res.json())
